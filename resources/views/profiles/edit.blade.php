...
<div class="user-profile-edition">
    <form action="{{route('profiles.update', ['user' => $user]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="user-profile-edition__fields fields">
            ...
            <div class="user-profile-edition__field box__bio field">
                ...
                <div class="control">
                    <textarea class="user-profile-edition__textarea textarea @error('biography') is-danger @enderror" rows="8" name="biography">{{$user->profile->biography}}</textarea>
                </div>
                @error('biography')
                <p class="help is-danger">{{ $message }}</p>
                @enderror
    ...
    <span class="user-profile-edition__modal-activation showModal" id="showModal">Supprimer mon compte</span>
    @include('partials.modals.deletion-modal', 
    ['modalTitle' => 'Supression du compte' ]
    )
</div>
